...
type StepId = "size" | "gifts" | "bonus" | "review";
...
export default function Home() {
  ...
  const [giftStageIndex, setGiftStageIndex] = useState(0);
  ...
  const currentGiftStage = giftStages[giftStageIndex];
  ...
  const activeStepIndex = useMemo(() => {
    ...
    return 3;
  }, [boxSize, giftBoxComplete, session?.wheel_reward]);
  ...
  }, [selectedIds.large, selectedIds.medium, selectedIds.small]);
  ...
  function continueFromGift() {
    if (currentGiftStage.id === "large" && selectedIds.large) {
      setGiftStageIndex(1);
      return;
    }

    if (currentGiftStage.id === "medium" && selectedIds.medium) {
      setGiftStageIndex(2);
      return;
    }

    if (currentGiftStage.id === "small" && selectedIds.small) {
      setStepIndex(2);
    }
  }
  ...
  const primaryAction = getPrimaryAction({
    step: currentStep.id,
    boxSize,
    currentGiftStage: currentGiftStage.id,
    selectedIds,
            ...
            {isLoading ? (
              ...
              <>
                ...
                <div className="border-t border-ink/10 p-4 sm:p-6">
                  <button
                    className="flex h-13 min-h-13 w-full items-center justify-center gap-2 rounded-md bg-ink px-5 py-4 text-base font-black text-white transition hover:bg-ink/90 disabled:cursor-not-allowed disabled:bg-ink/25"
  ...
  return (
    <section className="p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <StepIntro eyebrow="Step 2" title={currentGiftStage.title} description={currentGiftStage.description} />
            ...
            >
              ...
              <div className="flex min-h-32 flex-col justify-between gap-4 p-4">
                ...
                <div className="flex items-center justify-between gap-3">
                  <span className="rounded-full bg-[#f0ebe2] px-3 py-1 text-xs font-bold capitalize text-ink/65">{product.gender_target}</span>
    ...
    <aside className="hidden lg:block">
      <div className="sticky top-6 rounded-lg border border-ink/10 bg-white p-5 shadow-soft">
        ...
        <div className="mt-5 space-y-3">
          ...
          {giftStages.map((stage) => (
            <SummaryLine key={stage.id} label={stage.summaryLabel} value={selectedProducts[stage.id]?.name ?? nextGiftMessage(stage.id)} />
          ))}
...
function getPrimaryAction({
  ...
  if (step === "gifts") {
    const giftSelected = Boolean(selectedIds[currentGiftStage]);
    const label =
      currentGiftStage === "large"
        ? "Continue to second gift"
        : currentGiftStage === "medium"
