...
async def run_once(args, keywords: list) -> int:
    ...
    # One browser session for all keywords — same as original working script
    ...
    log.info("Saved payload to %s", payload_path.resolve())

    # Upload in chunks so pipeline can start reviewing the first batch
    # while the rest are still uploading
    ...
    for idx, chunk in enumerate(chunks, 1):
        try:
            ...
            log.info("Chunk %d/%d → job #%s (%d products accepted)", idx, len(chunks), job_id, accepted)
        except Exception as exc:
            log.error("Upload failed for chunk %d/%d: %s", idx, len(chunks), exc)

    log.info("Done — %d/%d products uploaded in %d chunk(s)", total_uploaded, len(products), len(chunks))
    return 0
